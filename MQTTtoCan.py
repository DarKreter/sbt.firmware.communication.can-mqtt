...
def callback(topic, threadValue):
    ...
    extID = canDecoder.encode_arbitrationID(keys[keys.size() - 3], keys[keys.size() - 2])    

    canPayload = canDecoder.encode_payload(keys[keys.size() - 2], keys[keys.size() - 1], threadValue)
